...
public class PeriodicLeaseSender implements Runnable, LeaseSender<LimitBasedLeaseTracker> {

  static final Logger logger = LoggerFactory.getLogger(PeriodicLeaseSender.class);

  final ConcurrentMap<LimitBasedLeaseTracker, FluxProcessor<Lease, Lease>>
      connectionsStatsToFluxSenders;
  final BlockingDeque<LimitBasedLeaseTracker> connectionsQueue;
  ...
  @Override
  public Flux<Lease> send(@Nullable LimitBasedLeaseTracker limitBasedLeaseTracker) {
  ...
  public void run() {
    try {
      LimitBasedLeaseTracker leaseStats = connectionsQueue.poll();
  ...
  void remove(LimitBasedLeaseTracker stats) {
