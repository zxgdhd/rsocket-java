...
public class FireAndForgetRequesterMonoTest {
  ...
  public void frameShouldBeSentOnSubscription(Consumer<FireAndForgetRequesterMono> monoConsumer) {
    ...
    final TestStreamManager streamManager = TestStreamManager.client();

    final FireAndForgetRequesterMono fireAndForgetRequesterMono =
        new FireAndForgetRequesterMono(
            allocator, payload, 0, FRAME_LENGTH_MASK, streamManager, sender);
    final StateMachineAssert<FireAndForgetRequesterMono> stateMachineAssert =
        StateMachineAssert.assertThat(FireAndForgetRequesterMono.STATE, fireAndForgetRequesterMono);
  ...
  public void frameFragmentsShouldBeSentOnSubscription(
    ...
    final FireAndForgetRequesterMono fireAndForgetRequesterMono =
        new FireAndForgetRequesterMono(
            allocator, payload, 64, FRAME_LENGTH_MASK, streamManager, sender);
    final StateMachineAssert<FireAndForgetRequesterMono> stateMachineAssert =
        StateMachineAssert.assertThat(FireAndForgetRequesterMono.STATE, fireAndForgetRequesterMono);
  ...
  public void shouldErrorIfNoAvailability(Consumer<FireAndForgetRequesterMono> monoConsumer) {
    final LeaksTrackingByteBufAllocator allocator =
        LeaksTrackingByteBufAllocator.instrument(ByteBufAllocator.DEFAULT);
    final UnboundedProcessor<ByteBuf> sender = new UnboundedProcessor<>();
    final TestStreamManager streamManager = TestStreamManager.client(new RuntimeException("test"));
    final Payload payload = ByteBufPayload.create("testData", "testMetadata");
    ...
    for (int i = 1; i < 50000; i += 2) {
      ...
      final StateMachineAssert<FireAndForgetRequesterMono> stateMachineAssert =
          StateMachineAssert.assertThat(
              FireAndForgetRequesterMono.STATE, fireAndForgetRequesterMono);
  ...
  public void checkName() {
    final LeaksTrackingByteBufAllocator allocator =
        LeaksTrackingByteBufAllocator.instrument(ByteBufAllocator.DEFAULT);
    final UnboundedProcessor<ByteBuf> sender = new UnboundedProcessor<>();
    final TestStreamManager testStreamManager = TestStreamManager.client();
    final Payload payload = ByteBufPayload.create("testData", "testMetadata");

    final FireAndForgetRequesterMono fireAndForgetRequesterMono =
        new FireAndForgetRequesterMono(
            allocator, payload, 0, FRAME_LENGTH_MASK, testStreamManager, sender);
